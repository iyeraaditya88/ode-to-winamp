  ...
  return (
    <div className="flex items-center gap-3 min-w-0">
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-[11px] text-white/45">{track.artists.map((a) => a.name).join(', ')}</p>
      </div>
    </div>
  );
}
...
export default function QueuePanel({ isOpen, onClose }: QueuePanelProps) {
  ...
  // Key the memo on the track *id*, not the object: the SDK recreates currentTrack
  // several times a second while playing, which would otherwise re-render the
  // thumbnails and make the panel flicker. Position updates don't touch these deps.
  ...
  const body = useMemo(() => {
    ...
    // currentTrack intentionally read via currentId (stable) to avoid re-render flicker.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentId, userQueue, dragIndex, overIndex, removeFromQueue, reorderQueue, upcoming]);
