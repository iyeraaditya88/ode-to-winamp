      ...
      {showNowPlaying && (
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4">
            <span className="text-xs font-mono tracking-[0.3em] text-white/40 uppercase">Now Playing</span>
            <button
              onClick={toggleNowPlaying}
              className="flex items-center gap-2 text-white/40 hover:text-white transition-colors text-xs font-mono tracking-widest"
            >
          ...
          <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-2 gap-8 px-6 lg:px-12 pb-8 overflow-hidden">
              ...
              <div className="w-full max-w-sm flex flex-col gap-4">
                ...
                <div className="flex items-center justify-center gap-6">
                  <button
                    ...
                    title="Shuffle"
                    className={`transition-colors ${shuffle ? 'text-[#00b4b4]' : 'text-white/40 hover:text-white/70'}`}
                  >
            ...
            <div className="flex flex-col min-h-0 gap-6">
              <div className="flex-1 min-h-0 flex flex-col">
                <span className="text-xs font-mono tracking-[0.3em] text-white/40 uppercase mb-3">Lyrics</span>
                <div className="flex-1 min-h-0 overflow-y-auto pr-2">
                  {!currentTrack && <p className="text-sm text-white/20 font-mono">Play a song to see lyrics</p>}
                  ...
                  {currentTrack && !isLoading && !hasSynced && !plainLyrics && (
                    <p className="text-sm text-white/20 font-mono">No lyrics found for this track</p>
                  )}
                  ...
                  {!hasSynced && plainLyrics && (
                    <pre className="text-base text-white/55 font-sans leading-relaxed whitespace-pre-wrap">
                      {plainLyrics}
                    </pre>
                  )}
                </div>
              ...
              {nextUp.length > 0 && (
                <div className="shrink-0 max-h-44 flex flex-col">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] text-white/25 font-mono">{nextUp.length} queued</span>
                  </div>
                    ...
                    {nextUp.map((t, i) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="truncate text-[10px] text-white/40">{t.artists.map((a) => a.name).join(', ')}</p>
                        </div>
                        <span className="text-[10px] text-white/25 font-mono">{fmt(t.duration_ms)}</span>
                      </button>
                    ))}
                  </div>
                </div>
